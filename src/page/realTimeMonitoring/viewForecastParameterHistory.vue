    ...
    <div class="leftDiv">
      ...
      <div class="selectedLegend">
        <el-checkbox-group v-model="modelParamCheckedList" @change="_handleCheckedModelParamChange" :max="5">
          <el-checkbox v-for="item in modelParamList" :label="item.name" :key="item.id" :style="{color : red}"
    ...
    methods: {
      _getToolTip(params) {
        ...
        if (this.type == 'output') {
          ...
          result = params.seriesName + '<br/>时间:' + params.name + '<br/>预测参数归一值:' + params.value + '<br/>在线参数归一化:' + this.forecastParameterHistory.allList.actualOutputData[params.dataIndex][index] + '<br/>lims归一值:' + gyz + '';
